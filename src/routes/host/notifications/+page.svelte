...
<div class="min-h-screen" in:fade={{ duration: 300 }}>
    <div class="flex items-center px-4 sm:px-8 md:px-12 py-6 md:py-8" 
         ...
         <h2 class="text-lg md:text-2xl font-bold pb-2 flex items-center text-[#F2F2F2]" in:fade={{ duration: 300 }}>
        ...
        {#each notifications as notification, i}
            ...
            >
                ...
                {:else if notification.message === "คำร้องซ่อมบำรุง"}
                    <img src="/images/greenBell.svg" alt="bell"
                         class="w-8 h-8 md:w-10 md:h-10 mr-3 md:mr-4 transition-transform hover:rotate-12"/>
                    <span class="absolute top-0 right-0 h-3 w-3 bg-green-500 rounded-full"></span>
                {:else}
                    <div class="relative">
